#!/usr/bin/env python3
"""检查中文成稿的硬禁令与常见模型化形状。只报警，不自动改文。

用法：
    python check_prose.py 稿件.md
    python check_prose.py -          # 从标准输入读

失败项必须清零才能发布。警告项需要回到材料和说话位置人工判断。

形状检测部分改写自 KKKKhazix/human-writing（MIT），
禁词表合并 vibe-writer-pro 自己的套话清单。
标点检查只作用于正文段落，标题、表格、列表、代码不受影响。
"""

from __future__ import annotations

import argparse
import collections
import re
import sys
from dataclasses import dataclass
from pathlib import Path


# ---------------------------------------------------------------- 硬失败项

HARD_STOPS = (
    "说白了",
    "说穿了",
    "先说结论",
    "不难发现",
    "综上所述",
    "总而言之",
    "值得一提的是",
    "不仅如此",
    "深入探讨",
)

HARD_JARGON = (
    "赋能",
    "抓手",
    "商业闭环",
    "价值闭环",
    "能力沉淀",
    "打法",
    "拉通",
    "底层逻辑",
    "顶层设计",
    "认知跃迁",
    "价值释放",
    "能力建设",
    "降本增效",
    "内容矩阵",
    "全链路",
    "组合拳",
    "打开想象空间",
    "想象空间",
    "结构性机会",
    "关键命题",
    "深层逻辑",
    "技术底座",
    "认知增量",
    "迭代闭环",
    "多重因素共振",
    "在当今",
    "随着社会的不断进步",
)

ROAD_SIGNS = (
    "更微妙的是",
    "还有一层",
    "只说对了一半",
    "值得注意的是",
    "需要指出的是",
    "从某种意义上说",
)

# 翻案句全族，7 种变体
PIVOT_PATTERNS = (
    re.compile(r"(?:并)?不是[^。！？\n]{0,90}而是"),
    re.compile(r"不是[^。！？\n]{1,24}[，,]\s*是"),
    re.compile(r"并非[^。！？\n]{0,90}而是"),
    re.compile(r"不在于[^。！？\n]{0,90}而在于"),
    re.compile(r"与其说[^。！？\n]{0,90}不如说"),
    re.compile(r"不只(?:是)?[^。！？\n]{0,90}(?:还|也)"),
    re.compile(r"表面(?:上)?[^。！？\n]{0,90}(?:其实|实际|实则|本质上)"),
    re.compile(r"看似[^。！？\n]{0,90}(?:其实|实际|实则)"),
)

FORBIDDEN_PUNCTUATION = {
    "：": "中文冒号",
    "—": "破折号",
    "–": "连接号式破折号",
}


# ---------------------------------------------------------------- 需人工判断

CONTEXT_JARGON = (
    "沉淀",
    "颗粒度",
    "对齐",
    "协同",
    "链路",
    "生态位",
    "心智",
    "范式",
    "方法论",
    "核心变量",
    "闭环",
)

SOFT_MARKERS = (
    "真正",
    "本质上",
    "更深层次",
    "归根结底",
    "换句话说",
    "不可否认",
    "核心是",
    "关键在于",
    "这意味着",
)

REPEATED_OPENERS = (
    "其实",
    "不过",
    "当然",
    "所以",
    "但是",
    "后来",
    "当时",
    "很多人",
    "问题是",
    "更重要的是",
    "说到这里",
)

LEFT_BRANCH_PATTERNS = (
    re.compile(r"(?:^|[。！？]\s*)在[^，。！？\n]{12,70}(?:以后|之后|之前|以前|过程中|情况下|背景下)，"),
    re.compile(r"(?:^|[。！？]\s*)那些[^，。！？\n]{10,60}的[^，。！？\n]{2,30}[，。]"),
    re.compile(r"(?:^|[。！？]\s*)(?:真正|最终|最后)让[^，。！？\n]{8,70}的，是"),
)

METAPHOR_FIELDS = {
    "温度": ("降温", "升温", "冷却", "余温", "温度最高"),
    "生死战争": ("杀死", "死因", "枪响", "开火", "战场", "引爆", "弹药"),
    "建筑灾害": ("坍塌", "崩塌", "地基", "砖头", "支柱", "废墟"),
    "仓储租赁": ("仓库", "库房", "租金", "取货", "入库"),
    "道路竞赛": ("赛道", "跑道", "岔路", "十字路口", "终点线", "门票"),
    "机器器官": ("齿轮", "引擎", "发动机", "血管", "骨架", "肌肉"),
    "海洋航行": ("蓝海", "浪潮", "潮水", "航船", "灯塔", "彼岸"),
}

# 用捕获组框住路标本身，行号才落在它真正出现的那一行，
# 而不是被前面的 \s* 拖回上一段的句号。
ROAD_SIGN_PATTERNS = tuple(
    re.compile(
        rf"(?:^|[。！？!?]\s*)({re.escape(phrase)}[^。！？!?\n]{{0,24}})",
        re.MULTILINE,
    )
    for phrase in ROAD_SIGNS
)


@dataclass
class Paragraph:
    position: int
    text: str
    han: int
    sentences: int


def han_count(text: str) -> int:
    return len(re.findall(r"[一-鿿]", text))


def line_number(text: str, position: int) -> int:
    return text.count("\n", 0, position) + 1


def excerpt(value: str, width: int = 72) -> str:
    value = re.sub(r"\s+", " ", value).strip()
    return value if len(value) <= width else value[: width - 1] + "…"


def _blank(match: re.Match[str]) -> str:
    """用空白替换，保留字符位置与换行，行号才不会错。"""
    return "".join("\n" if char == "\n" else " " for char in match.group())


def mask_non_prose(text: str) -> str:
    """屏蔽 frontmatter、代码、网址和 HTML 标签。"""
    patterns = (
        re.compile(r"\A---\s*\n.*?\n---\s*(?:\n|\Z)", re.DOTALL),
        re.compile(r"```.*?```", re.DOTALL),
        re.compile(r"`[^`\n]*`"),
        re.compile(r"!?\[[^\]\n]*\]\([^\n)]*\)"),
        re.compile(r"https?://[^\s)>]+"),
        re.compile(r"<[^>\n]+>"),
    )
    masked = text
    for pattern in patterns:
        masked = pattern.sub(_blank, masked)
    return masked


def mask_structure(text: str) -> str:
    """在 mask_non_prose 之上再屏蔽标题、表格、列表和引用标记。

    标点禁令只针对正文段落。小标题、表格和工具清单里的冒号是排版需要，
    不算 AI 味。
    """
    patterns = (
        re.compile(r"^#{1,6}[^\n]*$", re.MULTILINE),        # 标题
        re.compile(r"^\s*\|[^\n]*$", re.MULTILINE),          # 表格行
        re.compile(r"^\s*[-+*]\s[^\n]*$", re.MULTILINE),     # 无序列表
        re.compile(r"^\s*\d+[.、]\s[^\n]*$", re.MULTILINE),  # 有序列表
        re.compile(r"^\s*【[^\n】]*】[^\n]*$", re.MULTILINE), # 【】输出块标题
    )
    masked = text
    for pattern in patterns:
        masked = pattern.sub(_blank, masked)
    return masked


def non_overlapping_terms(text: str, terms: tuple[str, ...]):
    matches = []
    occupied = []
    for term in sorted(terms, key=len, reverse=True):
        for match in re.finditer(re.escape(term), text):
            start, end = match.span()
            if any(start < old_end and end > old_start for old_start, old_end in occupied):
                continue
            matches.append((start, term))
            occupied.append((start, end))
    return sorted(matches)


def all_matches(text: str, patterns):
    matches = []
    occupied = []
    for pattern in patterns:
        for match in pattern.finditer(text):
            start, end = match.span()
            if any(start < old_end and end > old_start for old_start, old_end in occupied):
                continue
            matches.append(match)
            occupied.append((start, end))
    return sorted(matches, key=lambda match: match.start())


def heavy_de_sentences(text: str):
    """找出主干可能被多个"的"压到后面的长句。"""
    matches = []
    pattern = re.compile(r"[^。！？!?\n]+(?:[。！？!?]|$)")
    for match in pattern.finditer(text):
        value = match.group()
        if han_count(value) >= 38 and value.count("的") >= 4:
            matches.append(match)
    return matches


def prose_paragraphs(text: str) -> list[Paragraph]:
    paragraphs = []
    cursor = 0
    for block in re.split(r"\n\s*\n", text):
        position = text.find(block, cursor)
        cursor = max(position + len(block), cursor)
        clean = re.sub(r"[>*_`]", "", block).strip()
        if not clean or clean.startswith(("#", "http", "![", "```", "|")):
            continue
        if re.match(r"^(?:[-+*]|\d+[.、])\s", clean):
            continue
        count = han_count(clean)
        if count < 4:
            continue
        sentences = max(1, len(re.findall(r"[。！？!?]", clean)))
        paragraphs.append(Paragraph(position, clean, count, sentences))
    return paragraphs


def metaphor_cluster(text: str, distance: int = 800):
    hits = []
    for field, words in METAPHOR_FIELDS.items():
        for word in words:
            for match in re.finditer(re.escape(word), text):
                hits.append((match.start(), field, word))
    hits.sort()
    for index, (start, _, _) in enumerate(hits):
        window = [hit for hit in hits[index:] if hit[0] - start <= distance]
        fields = {hit[1] for hit in window}
        if len(fields) >= 3:
            return window, fields
    return None


def short_streak(paragraphs: list[Paragraph], limit: int = 4):
    streak = []
    for paragraph in paragraphs:
        if paragraph.han <= 24 and paragraph.sentences <= 1:
            streak.append(paragraph)
            if len(streak) >= limit:
                return streak
        else:
            streak = []
    return None


def opener_counts(paragraphs: list[Paragraph]):
    counts = collections.Counter()
    examples = {}
    for paragraph in paragraphs:
        value = paragraph.text.lstrip("“‘\"（(")
        for opener in REPEATED_OPENERS:
            if value.startswith(opener):
                counts[opener] += 1
                examples.setdefault(opener, paragraph.position)
                break
    return counts, examples


def read_text(path: str) -> str:
    if path == "-":
        return sys.stdin.read()
    return Path(path).read_text(encoding="utf-8")


def main() -> int:
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")

    parser = argparse.ArgumentParser(description="检查中文成稿的硬禁令与模型化形状")
    parser.add_argument("path", help="Markdown 或文本文件路径。使用 - 从标准输入读取")
    args = parser.parse_args()

    try:
        text = read_text(args.path)
    except (OSError, UnicodeError) as error:
        print(f"无法读取稿件。{error}", file=sys.stderr)
        return 2

    prose = mask_non_prose(text)
    body = mask_structure(prose)
    total_han = han_count(prose)
    if total_han == 0:
        print("没有检测到汉字。", file=sys.stderr)
        return 2

    failures = []
    warnings = []

    # 标点只查正文段落
    for symbol, label in FORBIDDEN_PUNCTUATION.items():
        matches = list(re.finditer(re.escape(symbol), body))
        if matches:
            lines = "、".join(str(line_number(text, m.start())) for m in matches[:8])
            failures.append(f"正文{label}共 {len(matches)} 处，第 {lines} 行。改用逗号或句号。")

    stop_matches = non_overlapping_terms(prose, HARD_STOPS)
    for position, phrase in stop_matches:
        failures.append(f"套话，第 {line_number(text, position)} 行，{phrase}")

    jargon_matches = non_overlapping_terms(prose, HARD_JARGON)
    for position, phrase in jargon_matches:
        failures.append(f"黑话，第 {line_number(text, position)} 行，{phrase}")

    road_signs = all_matches(prose, ROAD_SIGN_PATTERNS)
    for match in road_signs:
        failures.append(
            f"模型路标，第 {line_number(text, match.start(1))} 行，"
            f"「{excerpt(match.group(1))}」"
        )

    pivots = all_matches(prose, PIVOT_PATTERNS)
    for match in pivots:
        failures.append(
            f"翻案句，第 {line_number(text, match.start())} 行，「{excerpt(match.group())}」"
        )

    # ---- 以下为警告，需人工判断

    hard_spans = [(p, p + len(w)) for p, w in jargon_matches]
    context_jargon = [
        (p, w)
        for p, w in non_overlapping_terms(prose, CONTEXT_JARGON)
        if not any(p < end and p + len(w) > start for start, end in hard_spans)
    ]
    if context_jargon:
        samples = "、".join(dict.fromkeys(w for _, w in context_jargon))
        lines = "、".join(
            dict.fromkeys(str(line_number(text, p)) for p, _ in context_jargon[:8])
        )
        warnings.append(
            f"有 {len(context_jargon)} 处词语要结合语境判断。第 {lines} 行出现 {samples}。"
            "本义准确时保留，用来给普通事情抬价时改写。"
        )

    marker_matches = non_overlapping_terms(prose, SOFT_MARKERS)
    marker_limit = max(2, total_han // 900)
    if len(marker_matches) > marker_limit:
        samples = "、".join(dict.fromkeys(t for _, t in marker_matches))
        warnings.append(
            f"洞察路标共 {len(marker_matches)} 处，提醒线 {marker_limit} 处。重点检查 {samples}。"
        )

    left_branches = all_matches(prose, LEFT_BRANCH_PATTERNS)
    left_limit = max(2, total_han // 1200)
    if len(left_branches) > left_limit:
        samples = "；".join(
            f"第 {line_number(text, m.start())} 行「{excerpt(m.group(), 44)}」"
            for m in left_branches[:4]
        )
        warnings.append(f"长前置成分 {len(left_branches)} 处，主干可能来得太晚。{samples}")

    dense_de = heavy_de_sentences(prose)
    dense_de_limit = max(1, total_han // 1500)
    if len(dense_de) > dense_de_limit:
        samples = "；".join(
            f"第 {line_number(text, m.start())} 行「{excerpt(m.group(), 44)}」"
            for m in dense_de[:4]
        )
        warnings.append(
            f"{len(dense_de)} 个长句包含 4 个以上的「的」，先交代人和动作。{samples}"
        )

    paragraphs = prose_paragraphs(prose)
    if len(paragraphs) >= 10:
        one_sentence = sum(p.sentences <= 1 for p in paragraphs)
        ratio = one_sentence / len(paragraphs)
        if ratio >= 0.75:
            warnings.append(
                f"可识别段落中 {ratio:.0%} 只有一句话，可能形成统一的短段鼓点。"
            )

    streak = short_streak(paragraphs)
    if streak:
        warnings.append(
            f"从第 {line_number(text, streak[0].position)} 行起连续 {len(streak)} 个短促单句段，"
            "检查是否在排队喊结论。"
        )

    counts, opener_examples = opener_counts(paragraphs)
    repeated = [(o, c) for o, c in counts.items() if c >= 4]
    if repeated:
        details = "、".join(f"{o} {c} 次" for o, c in repeated)
        first = min(opener_examples[o] for o, _ in repeated)
        warnings.append(f"段落开场重复，从第 {line_number(text, first)} 行附近开始。{details}。")

    metaphors = metaphor_cluster(prose)
    if metaphors:
        window, fields = metaphors
        samples = "、".join(dict.fromkeys(h[2] for h in window))
        warnings.append(
            f"800 字内出现 {len(fields)} 套借喻（{'、'.join(sorted(fields))}）。例词 {samples}。"
            "先全部还原成本义，本义清楚就一个也不用放回。"
        )

    print(f"汉字数 {total_han}")
    print(
        f"翻案句 {len(pivots)}，黑话 {len(jargon_matches)}，套话 {len(stop_matches)}，"
        f"模型路标 {len(road_signs)}，需辨语境词 {len(context_jargon)}，"
        f"洞察路标 {len(marker_matches)}，长前置成分 {len(left_branches)}，"
        f"重定语句 {len(dense_de)}"
    )

    if failures:
        print("\n需要修改")
        for item in failures:
            print(f"- {item}")

    if warnings:
        print("\n需要人工判断")
        for item in warnings:
            print(f"- {item}")

    if not failures and not warnings:
        print("\n未发现这份检查器覆盖的问题。")

    return 1 if failures else 0


if __name__ == "__main__":
    raise SystemExit(main())
